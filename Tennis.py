import tkinter
# а модуль messagebox необходимо подгрузить отдельно из tkinter
...
canvas.pack ()
# создаем размеры ракетки и мячика
bat = canvas.create_rectangle (0,0,40,10, fill='dark turquoise')
ball = canvas.create_oval (0,0,10,10, fill='deep pink')
# здесь создаем главный цикл, в котором будут отслеживаться все изменения на игровом поле
windowOpen = True
def main_loop ():
    while windowOpen == True:
        ...
        if windowOpen == True:
            check_game_over ()
# тут мы подключаем стрелки на клавиатуре
leftPressed = 0
rightPressed = 0
## нажатие клавиши
